...
poetry run pytest                        # all tests (e2e-marked tests excluded by default); also runs ruff, ty, semgrep, vulture, lizard, coverage-floor, and cpd checks as pytest tests
...
Linters are not run through a bespoke CLI -- `tests/integration/test_lint.py` and `tests/test_ci_gate.py` invoke ruff, ty, semgrep, vulture, lizard, coverage, and cpd (`npx jscpd`) directly as pytest tests, reading thresholds from `config/lint.toml`. CPD checks require Node.js. Audio tooling requires `ffmpeg` on PATH.
...
- `part_io/models/` -- pure data; currently just `models/ports/`, which defines callable/Protocol port types (`AuditorFn`). May import only `part_io.models.*`. No print, subprocess, or file I/O.
- `part_io/services/` -- orchestration logic (e.g. `audio_bootstrap`, the human-in-the-loop jingle discovery pipeline). Receives port implementations as function arguments; may import only `part_io.services.*` and `part_io.models.*`. No print, subprocess, or file I/O.
- `part_io/adapters/` -- implementations of the ports: process runner (`process/`) and the audio matcher/evaluation (`audio/`). May import `part_io.adapters.*`, `part_io.models.*`, `part_io.utils.*`.
...
- `part_io/cli/` -- entrypoints only. This is the only layer allowed to call `print()` and `sys.exit()`. Currently just the `audio_*.py` CLIs.
...
The audio sample matching domain: `adapters/audio/matcher.py` decodes audio to mono 16 kHz PCM via ffmpeg and matches a reference sample against a longer recording using 32-band spectral-energy features plus deltas. `cli/audio_search.py` prints match timestamps; `cli/audio_review.py` extracts MP3 clips and writes a review manifest. `adapters/audio/evaluation.py` scores labeled review results. `services/audio_bootstrap.py` plus `cli/audio_bootstrap.py` handle cold-start jingle discovery when no reference sample exists yet.
