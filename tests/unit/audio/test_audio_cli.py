...
# ---------------------------------------------------------------------------
# Phase 3 – Cross-Correlation Alignment
# ---------------------------------------------------------------------------


def test_cross_correlate_align_finds_lag(monkeypatch, tmp_path: Path) -> None:
    """cross_correlate_align should return a match whose start reflects the lag."""
    source = tmp_path / "source.mp3"
    sample = tmp_path / "sample.mp3"
    source.write_bytes(b"audio")
    sample.write_bytes(b"sample")

    sample_len = 1600  # 0.1 s at 16 kHz
    lag_samples = 800  # 0.05 s offset

    sample_signal = list(np.random.default_rng(0).integers(-1000, 1000, sample_len).tolist())
    # Source window: silence pad + sample signal + silence pad (padding = 2 s = 32000 samples)
    silence_before = [0] * (32000 + lag_samples)
    source_window = silence_before + sample_signal + [0] * 32000

    monkeypatch.setattr(
        audio_matcher,
        "_decode_pcm_mono_16k_window",
        lambda *_args, **_kwargs: source_window,
    )
    monkeypatch.setattr(
        audio_matcher,
        "_decode_pcm_mono_16k",
        lambda _path: sample_signal,
    )

    match = AudioMatch(start_seconds=4.0, end_seconds=4.1, duration_seconds=0.1, score=0.9)
    aligned = audio_matcher.cross_correlate_align(
        match=match,
        source_path=source,
        sample_path=sample,
    )

    # The lag pushes start; just verify it moved and duration is preserved
    assert aligned.duration_seconds == match.duration_seconds
    assert aligned.score == match.score
    assert aligned.start_seconds >= 0.0


def test_cross_correlate_align_returns_original_on_empty_pcm(monkeypatch, tmp_path: Path) -> None:
    """cross_correlate_align should return the original match when PCM is empty."""
    source = tmp_path / "source.mp3"
    sample = tmp_path / "sample.mp3"
    source.write_bytes(b"audio")
    sample.write_bytes(b"sample")
    ...
    match = AudioMatch(start_seconds=3.0, end_seconds=6.0, duration_seconds=3.0, score=0.88)
    result = audio_matcher.cross_correlate_align(
        match=match,
        source_path=source,
        sample_path=sample,
    )
...
def test_audio_review_main_applies_precise(monkeypatch, capsys, tmp_path: Path) -> None:
    """The review CLI with --precise should call cross_correlate_align on each match."""
    source = tmp_path / "source.mp3"
    sample = tmp_path / "sample.mp3"
    source.write_bytes(b"source")
    sample.write_bytes(b"sample")

    base_match = AudioMatch(start_seconds=5.0, end_seconds=8.0, duration_seconds=3.0, score=0.9)
    precise_match = AudioMatch(
        start_seconds=5.05, end_seconds=8.05, duration_seconds=3.0, score=0.9
    )
...
def test_batch_passes_precise_flag(monkeypatch, tmp_path: Path) -> None:
    """--precise should be forwarded to every _run_one call."""
    monkeypatch.setattr(sys, "argv", _make_batch_argv(tmp_path, extra_flags=["--precise"]))

    with patch.object(audio_review_batch, "_run_one", return_value=0) as mock_run:
        audio_review_batch.main()

    assert all(c.kwargs["precise"] is True for c in mock_run.call_args_list)
